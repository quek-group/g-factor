...
SUBROUTINE c_bands_nscf( )
  ...
  USE wvfct,                ONLY : et, nbnd, npwx, current_k,nbndx
  ...
  !
  COMPLEX(DP) :: vectx(2*npwx),vecty(2*npwx),pupkx(2*npwx),pupky(2*npwx),overlapx,overlapy
...
! Xuan Fengyuan modify the code for g factor calculation starts here
     ...
     IF ( ik .eq. 1 ) THEN
     WRITE( stdout, * ) 'ik ngk,npol',ik,ngk(ik),npol
     WRITE( stdout, * ) ' Shape evc igk_k',SHAPE(evc),SHAPE(igk_k)
     phaser = 0.0d0
     allocate(xpsi(2*npwx,1,nbndx), STAT=ierr)
     allocate(xhpsi(2*npwx,1,nbndx), STAT=ierr)
  xhpsi = ( 0.D0, 0.D0 )
  xpsi  = ( 0.D0, 0.D0 )
  xpsi(:,1,1:nbnd) = evc(:,1:nbnd)
     ...
     FLUSH(stdout)
     WRITE( stdout, * ) 'xpsi',xpsi(100,1,iiiv),xpsi(100+npwx,1,iiiv)
     FLUSH(stdout)
  CALL allocate_bec_type ( nkb, nbnd, becp, intra_bgrp_comm )
  ...
  CALL deallocate_bec_type ( becp )
     ...
     WRITE( stdout, * ) xhpsi(100+npwx,1,iiiv)/xpsi(100+npwx,1,iiiv)
     deallocate (xpsi)
     deallocate (xhpsi)
     END IF
     IF ( ik .eq. 2 ) THEN
     WRITE( stdout, * ) 'ik ngk',ik,ngk(ik)
     phaser = 0.0d0
       phasei = 0.0d0
       phasery = 0.0d0
       phaseiy = 0.0d0
     ...
     vecty(iiii) = evc(iiii,iiiv)
      phaser = phaser + REAL(evc(iiii,iiiv))**2+AIMAG(evc(iiii,iiiv))**2
     end do
     do iiii = 1,npwx
      phasei = phasei + REAL(evc(iiii,iiiv))**2+AIMAG(evc(iiii,iiiv))**2
      phaseiy = phaseiy + REAL(evc(iiii+npwx,iiiv))**2+AIMAG(evc(iiii+npwx,iiiv))**2
     end do
     WRITE( stdout, * ) 'ik=2 normalized? total 1st 2nd',phaser,phasei,phaseiy
     END IF
      FLUSH(stdout)
     IF ( ik .eq. 3 ) THEN
     WRITE( stdout, * ) 'ik ngk',ik,ngk(ik)
...
!       WRITE(stdout,*) qy
     ...
     do iiii = 1,2*npwx
      ...
      iiiy = 0
      do iiij = 1,npwx
       if(iiii .le. npwx) then
        if (igk_k(iiij,1) .eq. igk_k(iiii,3)) iiix = iiij
        if (igk_k(iiij,2) .eq. igk_k(iiii,3)) iiiy = iiij 
       else
        if (igk_k(iiij,1) .eq. igk_k(iiii-npwx,3)) iiix = iiij+npwx
        if (igk_k(iiij,2) .eq. igk_k(iiii-npwx,3)) iiiy = iiij+npwx
       endif
      end do
...
!      berrym=berrym-0.0987*berrycurv
      berrym=-berrym/2.0d0
     ...
     deallocate (xhpsi)
     END IF
! end of the modificationfor g factor calculation
     !
     IF ( io_level > -1 ) CALL save_buffer ( evc, nwordwfc, iunwfc, ik )
     ! ... beware: with pools, if the number of k-points on different
     ! ... pools differs, make sure that all processors are still in
